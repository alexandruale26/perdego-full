...
const BreadcrumbList = forwardRef(({ className, ...props }, ref) => (
  <ol
    ref={ref}
    className={cn(
      "flex flex-wrap items-center gap-0.5 break-words text-lg text-grey-3",
      className,
    )}
    {...props}
  />
...
const BreadcrumbLink = forwardRef(({ asChild, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn(
        "px-1 transition-colors hover:text-black focus-visible:outline-secondary",
        className,
      )}
      {...props}
    />
...
const BreadcrumbPage = forwardRef(({ className, ...props }, ref) => (
  <span
    ...
    aria-disabled="true"
    aria-current="page"
    className={cn("text-black px-1", className)}
    {...props}
  />
));
